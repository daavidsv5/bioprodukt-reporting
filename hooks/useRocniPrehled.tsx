...
interface RocniPrehledCtx {
  ...
  /** Trh (Vše / CZ / SK) */
  country: RocniCountry;
  setCountry: (c: RocniCountry) => void;
}

export type RocniCountry = 'all' | 'cz' | 'sk';

const Ctx = createContext<RocniPrehledCtx | null>(null);

export function RocniPrehledProvider({ children }: { children: React.ReactNode }) {
  const yearInfos = useMemo(() => getYearInfos([...realDataCZ, ...realDataSK]), []);
  ...
  const [country, setCountry] = useState<RocniCountry>('all');
  ...
  return (
    <Ctx.Provider value={{ yearInfos, selectedYears: selected, toggleYear, country, setCountry }}>
      {children}
    </Ctx.Provider>
  );
}

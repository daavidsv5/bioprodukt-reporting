...
function aggregateByMonth(
  ...
  yearStr: string,
): Map<number, { day: AggDay; margin: AggMargin }> {
  const result = new Map<number, { day: AggDay; margin: AggMargin }>();
  ...
  for (const r of marginData) {
    ...
    const m = parseInt(r.date.substring(5, 7), 10);
    const acc = result.get(m)!;
    acc.margin.purchaseCost += r.purchaseCost;
    acc.margin.revenue      += r.revenue;
  }

  return result;
}

function toPoint(agg: { day: AggDay; margin: AggMargin } | undefined): {
  revenue: number | null; cost: number | null; pno: number | null;
  aov: number | null; marginPct: number | null; cpa: number | null; orders: number | null;
} {
  if (!agg || agg.day.orders === 0) {
    // Pokud není margin ale je revenue, vrátíme alespoň to
    ...
    return { revenue: null, cost: null, pno: null, aov: null, marginPct: null, cpa: null, orders: null };
  }
...
export function useMainDashboard(country: MainCountry, year: number): MonthlyPoint[] {
  return useMemo(() => {
    ...
    const curr = aggregateByMonth(realData, marginData, currStr);
    const prev = aggregateByMonth(realData, marginData, prevStr);

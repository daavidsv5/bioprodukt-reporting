...
const SHEETS = {
  orders_cz: 'https://docs.google.com/spreadsheets/d/e/2PACX-1vToMQ4iS5Vh1I8XwTqj9EMn-gz8BYscQgsD7wBylZkPTQbjQjLs37RW6yzHv8-4gx96SDPRfUMD7WJi/pub?output=csv',
  cost_cz:   'https://docs.google.com/spreadsheets/d/e/2PACX-1vT92aQpUbd6C3BC7BKwOVvyo64JVhITwzMDkouT6tThNabO9AUrXyyP6q488eV6QBpfjLqKDhCcFhxq/pub?output=csv',
  margin_cz: 'https://docs.google.com/spreadsheets/d/e/2PACX-1vQpw9HZDLdv6f-_shJaicT1dmAWczBmsUNmplBruRpRh6U605Wku1o2itD4W8B4uKwCS04RUDB6fT-4/pub?output=csv',
  stock_cz:  'https://docs.google.com/spreadsheets/d/e/2PACX-1vRXaVfFIzh-hrFomvzwElGagHX79st58r0H-Yttrhu0fnHHpyh4e4rhnHKQl7ElFyewkeIC6_fCBM5t/pub?output=csv',
};

const DATA_DIR       = path.join(__dirname, '..', 'data');
const LOG_FILE       = path.join(__dirname, 'updateData.log');
const EUR_TO_CZK_FALLBACK = 25;
...
async function fetchEurToCzk() {
  try {
    const json = await fetchUrl('https://api.frankfurter.app/latest?from=EUR&to=CZK');
    const rate = JSON.parse(json)?.rates?.CZK;
    if (rate && typeof rate === 'number' && rate > 1) {
      log(`EUR→CZK live rate: ${rate}`);
      return rate;
    }
    throw new Error('Neplatná odpověď');
  } catch (e) {
    log(`EUR→CZK fetch failed (${e.message}), using fallback ${EUR_TO_CZK_FALLBACK}`);
    return EUR_TO_CZK_FALLBACK;
  }
}
...
function isPaymentName(name) {
  ...
  if (n === 'převodem' || n === 'prevodem' || n.startsWith('převodem') || n.startsWith('prevodem')) return true; // Převodem
...
function aggregateOrders(csv, eurMultiplier = 1, colFilter = null) {
  ...
  for (const cols of rows) {
    ...
    if (colFilter && !colFilter(cols)) continue;
    const code   = cols[0];
    const status = cols[2];
...
// ── Product processing ────────────────────────────────────────────────────────
function aggregateProducts(csv, eurMultiplier = 1, colFilter = null) {
  const rows = parseCSV(csv);
...
function writeTsFile(filePath, varName, interfaceName, currencyComment, records, countryFallback = 'cz') {
  const today = new Date().toISOString().split('T')[0];
  const content = `// Auto-generated by scripts/updateData.js — last update: ${today}
// ${currencyComment}
...
export interface ${interfaceName} {
  date: string;
  country: '${records[0]?.country || countryFallback}';
  orders: number;
...
// codeFilter: optional Set<string> — pokud je zadán, zpracují se pouze kódy v Setu
function aggregateMargin(csv, codeFilter = null) {
  const rows = parseCSV(csv);
  const seenCodes = new Set();
  const byDay = {};
  ...
  for (const cols of rows) {
    ...
    if (codeFilter !== null && !codeFilter.has(code)) continue;
...
function aggregateHourly(csv, colFilter = null) {
  const rows = parseCSV(csv);
...
function aggregateCrossSell(csv, colFilter = null) {
  const rows = parseCSV(csv);
  ...
  // Group distinct product names by order code, track order month
  const orderData = new Map(); // code -> { month, products: Set }
  for (const cols of rows) {
    if (cols.length < 57) continue;
    if (colFilter && !colFilter(cols)) continue;
    const code = cols[0];
    ...
    if (!orderData.has(code)) orderData.set(code, { month: cols[1].substring(0, 7), products: new Set() });
    orderData.get(code).products.add(name);
  }

  // Aggregate by month
  const byMonth = new Map(); // month -> { totalOrders, multiItemOrders, pairCounts: Map }
  for (const { month, products } of orderData.values()) {
    if (!byMonth.has(month)) byMonth.set(month, { totalOrders: 0, multiItemOrders: 0, pairCounts: new Map() });
    ...
    for (let i = 0; i < arr.length; i++) {
      for (let j = i + 1; j < arr.length; j++) {
        ...
        m.pairCounts.set(key, (m.pairCounts.get(key) || 0) + 1);
      }
    }
  }

  // Build sorted monthly records, top 300 pairs per month for range re-ranking
  return [...byMonth.entries()]
    .sort((a, b) => a[0].localeCompare(b[0]))
    .map(([month, { totalOrders, multiItemOrders, pairCounts }]) => ({
      month,
      totalOrders,
      multiItemOrders,
      pairs: [...pairCounts.entries()]
        ...
        .map(([key, count]) => {
          ...
          return { productA: key.slice(0, sep), productB: key.slice(sep + 3), count };
        }),
    }));
}

function writeCrossSellTsFile(filePath, varName, country, records) {
  const today = new Date().toISOString().split('T')[0];
  const content = `// Auto-generated by scripts/updateData.js — last update: ${today}
// ${country.toUpperCase()}: product pair co-occurrence by month (top 300 pairs/month for date-range filtering)
...
export const ${varName}: CrossSellMonthlyRecord[] = ${JSON.stringify(records, null, 2)};
`;
  fs.writeFileSync(filePath, content, 'utf8');
}
...
function aggregateOrderValues(csv, colFilter = null) {
  const rows = parseCSV(csv);
...
function aggregateRetention(csv, colFilter = null) {
  const rows = parseCSV(csv);
...
async function main() {
  ...
  try {
    ...
    const EUR_TO_CZK = await fetchEurToCzk();
    ...
    // ── Filtry podle currency a campaign ──────────────────────────────────────
    const isCZ = cols => cols[3] === 'CZK';
    const isSK = cols => cols[3] === 'EUR';
    const isCZCost = cols => (cols[7] || '').toLowerCase().startsWith('cz-');
    const isSKCost = cols => (cols[7] || '').toLowerCase().startsWith('sk-');

    // ── CZ ────────────────────────────────────────────────────────────────────
    const ordersByDayCZ = aggregateOrders(csvOrdersCZ, 1, isCZ);
    const { byDay: costByDayCZ, byDaySource: costSrcCZ } = aggregateCost(csvCostCZ, 1, isCZCost);
    ...
    const productsCZ = aggregateProducts(csvOrdersCZ, 1, isCZ);
    ...
    // ── SK ────────────────────────────────────────────────────────────────────
    // SK objednávky jsou v EUR, identifikujeme je podle sloupce currency
    // SK náklady jsou v cost sheetu kampaně začínající 'sk-'
    const ordersByDaySK = aggregateOrders(csvOrdersCZ, 1, isSK); // EUR, z téhož CSV
    const { byDay: costByDaySK, byDaySource: costSrcSK } = aggregateCost(csvCostCZ, 1 / EUR_TO_CZK, isSKCost); // CZK→EUR
    ...
    log(`SK: ${recordsSK.length} days | ${totalSK.orders} orders | ${totalSK.revenue_vat.toFixed(2)} € | PNO ${(totalSK.cost / (recordsSK.reduce((s,r) => s+r.revenue, 0) || 1) * 100).toFixed(2)}%`);
    ...
    const productsSK = aggregateProducts(csvOrdersCZ, 1, isSK);
    ...
    // ── CZ Shipping / Payment ─────────────────────────────────────────────────
    const shippingPaymentCZ = aggregateShippingPayment(csvOrdersCZ, 1, isCZ);
    ...
    // ── SK Shipping / Payment ─────────────────────────────────────────────────
    const shippingPaymentSK = aggregateShippingPayment(csvOrdersCZ, 1, isSK);
    ...
    const orderValuesCZ = aggregateOrderValues(csvOrdersCZ, isCZ);
    ...
    const orderValuesSK = aggregateOrderValues(csvOrdersCZ, isSK);
    ...
    // ── CZ retention ──────────────────────────────────────────────────────────
    const retentionCZ = aggregateRetention(csvOrdersCZ, isCZ);
    ...
    // ── SK retention ──────────────────────────────────────────────────────────
    const retentionSK = aggregateRetention(csvOrdersCZ, isSK);
    ...
    // ── CZ hourly behaviour ───────────────────────────────────────────────────
    const hourlyCZ = aggregateHourly(csvOrdersCZ, isCZ);
    ...
    // ── SK hourly behaviour ───────────────────────────────────────────────────
    const hourlySK = aggregateHourly(csvOrdersCZ, isSK);
    ...
    // ── CZ cross-sell ─────────────────────────────────────────────────────────
    const crossSellCZ = aggregateCrossSell(csvOrdersCZ, isCZ);
    writeCrossSellTsFile(path.join(DATA_DIR, 'crossSellDataCZ.ts'), 'crossSellDataCZ', 'cz', crossSellCZ);
    log(`CZ cross-sell: ${crossSellCZ.length} months | ${crossSellCZ.reduce((s, m) => s + m.totalOrders, 0)} orders`);

    // ── SK cross-sell ─────────────────────────────────────────────────────────
    const crossSellSK = aggregateCrossSell(csvOrdersCZ, isSK);
    writeCrossSellTsFile(path.join(DATA_DIR, 'crossSellDataSK.ts'), 'crossSellDataSK', 'sk', crossSellSK);
    log(`SK cross-sell: ${crossSellSK.length} months | ${crossSellSK.reduce((s, m) => s + m.totalOrders, 0)} orders`);

    // ── Sestav Set kódů SK objednávek z orders exportu ────────────────────────
    const skOrderCodes = new Set();
    for (const cols of parseCSV(csvOrdersCZ)) {
      if (cols.length > 3 && cols[3] === 'EUR') skOrderCodes.add((cols[0] || '').trim());
    }
    log(`SK order codes from orders export: ${skOrderCodes.size}`);

    // ── CZ Margin (kódy které NEJSOU v SK setu) ───────────────────────────────
    const czOrderCodes = new Set();
    for (const cols of parseCSV(csvOrdersCZ)) {
      if (cols.length > 3 && cols[3] === 'CZK') czOrderCodes.add((cols[0] || '').trim());
    }
    const marginRecordsCZ = aggregateMargin(csvMarginCZ, czOrderCodes);
    ...
    // ── SK Margin (kódy které jsou v SK setu, hodnoty v EUR) ──────────────────
    const marginRecordsSK = aggregateMargin(csvMarginCZ, skOrderCodes);
    writeMarginTsFile(path.join(DATA_DIR, 'marginDataSK.ts'), 'marginDataSK', 'EUR', marginRecordsSK);
    const totalMarginSK = marginRecordsSK.reduce((s, r) => s + (r.revenue - r.purchaseCost), 0);
    if (marginRecordsSK.length > 0) {
      log(`SK margin: ${marginRecordsSK.length} days | marže ${totalMarginSK.toFixed(2)} EUR`);
    } else {
      log('SK margin: žádné záznamy nalezeny v margin sheetu (kódy SK objednávek nenalezeny)');
    }
    ...
    // ── SK Stock ──────────────────────────────────────────────────────────────
    writeStockTsFile(path.join(DATA_DIR, 'stockDataSK.ts'), 'stockDataSK', 'sk', []);
    log('SK stock: není k dispozici');

    // ── Last update timestamp ─────────────────────────────────────────────────
    const nowIso = new Date().toISOString();
    fs.writeFileSync(
      path.join(DATA_DIR, 'lastUpdate.ts'),
      `// Auto-generated by scripts/updateData.js\nexport const LAST_UPDATE = '${nowIso}';\n`,
      'utf8'
    );

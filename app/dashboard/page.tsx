...
export default function DashboardPage() {
  ...
  // LTV (bez DPH) — lifetime, per-customer, all-time (nezávisí na period filtru)
  ...
  // CZ nákupní ceny existují až od CZ_PURCHASE_COST_FROM — dřívější marže vychází 100 %,
  // takže metriky z marže (vč. YoY) jsou pro takové období nadhodnocené → upozornění na boxech
  const beforePurchaseCosts = (from: Date) => from.toISOString().split('T')[0] < CZ_PURCHASE_COST_FROM;
  const marginNote = filters.countries.includes('cz') && (beforePurchaseCosts(start) || (hasPrevData && beforePurchaseCosts(prevStart)))
    ? 'CZ bez nákupních cen před 12/2024 – marže nadhodnocená'
    : undefined;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        <PoasChart data={poasChartData} hasPrevData={hasPrevData} note={marginNote} />
